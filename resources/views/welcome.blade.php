<x-guest-layout>
    ...
    <x-authentication-card>
        ...
        <style>
            .bg-input {
                background-color: hsla(0, 0%, 100%, 0.274) !important;}
        </style>
        <form style= 'width: 300px;' method="POST" action="{{ route('login') }}">
            ...
            <div class="mt-10">
                ...
                <x-input id="email" class="bg-input block mt-1 w-full" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
            </div>

            <div class="mt-4">
                <x-label for="password" value="{{ __('Contraseña') }}" />
                <x-input id="password" class="bg-input block mt-1 w-full" type="password" name="password" required autocomplete="current-password" />
            </div>

            <div class="block mt-4">
                <label for="remember_me" class="flex items-center">
                    <x-checkbox class="bg-input" id="remember_me" name="remember" />
            ...
            <div class="flex text-center items-center  mt-4">
                <x-button class="w-full items-center">
                    <h1 style= 'font-size:18px'>INGRESAR</h1>
                </x-button>
            </div>
            <div class="mt-4">
